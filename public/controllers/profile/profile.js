...
async function delete_(id) {
     method = 'GET';
     url = URI_PROFILE + LIST_CRUD[3] + '/' + id;
     data = "";
...
mainApp.getForm().addEventListener("submit", async function(event) {
    ...
    if (mainApp.setValidateForm()) {
        ...
        if (insertUpdate) {
            // Insertion branch
             method = "POST";
             url = URI_PROFILE + LIST_CRUD[0]; // Replace 0 with the appropriate index for insertion if needed
             data = mainApp.getDataFormJson();
             resultFetch = getData(data, method, url);
            ...
            // Update branch
             method = "POST";
             url = URI_PROFILE + LIST_CRUD[2]; // Update endpoint
             data = mainApp.getDataFormJson();
             resultFetch = getData(data, method, url);
